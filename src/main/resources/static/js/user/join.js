    ...
    axios.patch('/user/sendMail', formData)
        .then(res => {
            if (res.data === 'SUCCESS') {
                ...
                joinForm['emailVerifyBtn'].setAttribute('disabled', '');
...
joinForm['confirmDuplicationBtn'].onclick = function (e) {
    ...
    axios.get("/user/confirmDuplication?nickname=" + joinForm['nickname'].value)
        .then(res => {
            if (res.data === 'FAILURE_DUPLICATED_NICKNAME') {
                alert("이미 존재하는 닉네임 입니다. 다른 닉네임을 입력해 주세요.");
            } else if (res.data === 'SUCCESS') {
                alert("사용 가능한 닉네임 입니다.");
                joinForm['nickname'].classList.add("confirmed");
            }
        })
        .catch(err => {
            console.log(err);
